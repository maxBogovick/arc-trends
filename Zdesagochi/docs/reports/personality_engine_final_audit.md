...
It is not yet a full server-authoritative LiveOps system. Backend replay/validation, remote registry endpoint, server economy confirmation, broader Monte Carlo balance sweeps and product UI polish remain explicit post-MVP work.
...
| MVP-2. Offline shell without gameplay logic | `PetService` wraps command execution; `LocalSave`, `SyncQueue`, `ServerApi` split runtime concerns; `mockApi` delegates command path | Real backend replay/validation and server confirmation are not implemented |
...
| M4. System influences | Partial | Auto-sleep is command-owned; generic `system:*` / `env:*` registry sync pass remains |
...
- Backend replay/validation adapter using the same command outcome.
- Server-side economy/inventory/rewards confirmation.
- Generic `system:*` and `env:*` influence pass during sync.
