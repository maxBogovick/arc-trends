...
pub async fn spend_coins(pool: &PgPool, user_id: &str, amount: i32) -> Result<i32, AppError> {
    ...
    .fetch_optional(pool)
    .await?;
    row.map(|r| r.0).ok_or_else(|| AppError::BadRequest("Недостаточно монет".into()))
}
...
pub async fn add_inventory(pool: &PgPool, user_id: &str, item_id: &str, qty: i32) -> Result<(), AppError> {
    sqlx::query(
...
pub async fn use_inventory_item(pool: &PgPool, user_id: &str, item_id: &str) -> Result<(), AppError> {
    let row = sqlx::query_as::<_, (i32,)>(
    ...
    .fetch_optional(pool)
    .await?;
    row.map(|_| ()).ok_or_else(|| AppError::NotFound(format!("Предмет {} не найден в инвентаре", item_id)))
}
...
pub async fn add_inventory_tx(tx: &mut Tx<'_>, user_id: &str, item_id: &str, qty: i32) -> Result<(), AppError> {
    sqlx::query(
        "INSERT INTO user_inventory (user_id, item_id, quantity) VALUES ($1, $2, $3)
         ON CONFLICT (user_id, item_id) DO UPDATE SET quantity = user_inventory.quantity + $3"
    )
...
pub async fn use_inventory_item_tx(tx: &mut Tx<'_>, user_id: &str, item_id: &str) -> Result<(), AppError> {
    let row = sqlx::query_as::<_, (i32,)>(
        "UPDATE user_inventory SET quantity = quantity - 1
         WHERE user_id = $1 AND item_id = $2 AND quantity > 0
         RETURNING quantity"
