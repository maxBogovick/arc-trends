use crate::{
    domain::rooms::{LeaderboardEntry, Room},
    engine::catalog,
    error::AppError,
};
...
pub async fn get_rooms(pool: &PgPool, user_id: &str) -> Result<Vec<Room>, AppError> {
    ...
    let rows =
        sqlx::query_as::<_, Row>("SELECT room_id, unlocked FROM user_rooms WHERE user_id = $1")
            .bind(user_id)
            .fetch_all(pool)
            .await?;
    ...
    let rooms = defs
        .into_iter()
        .map(|def| {
            let unlocked = rows.iter().any(|r| r.room_id == def.id && r.unlocked) || def.price == 0;
...
pub async fn equip_room(pool: &PgPool, user_id: &str, room_id: &str) -> Result<(), AppError> {
    ...
    let is_default = catalog::rooms_catalog()
        .iter()
        .any(|r| r.id == room_id && r.price == 0);
    ...
    sqlx::query("UPDATE user_rooms SET equipped = (room_id = $2) WHERE user_id = $1")
        .bind(user_id)
        .bind(room_id)
        .execute(pool)
        .await?;
