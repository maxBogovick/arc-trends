...
use crate::{domain::progress::{Achievement, AchievementDef, DailyQuest, QuestDef}, engine::catalog, error::AppError};
use chrono::Utc;
...
pub async fn get_achievements(pool: &PgPool, user_id: &str) -> Result<Vec<Achievement>, AppError> {
    ...
    #[derive(sqlx::FromRow)]
    struct Row { achievement_id: String, progress: i32, unlocked: bool, unlocked_at: Option<chrono::DateTime<Utc>>, claimed: bool }
    ...
    for def in defs {
        ...
        let (progress, unlocked, unlocked_at, claimed) = row
            .map(|r| (r.progress, r.unlocked, r.unlocked_at.map(|t| t.to_rfc3339()), r.claimed))
            .unwrap_or((0, false, None, false));
        out.push(Achievement {
            id: def.id, name: def.name, description: def.description, emoji: def.emoji,
            category: def.category, progress, target: def.target,
            unlocked, unlocked_at, claimed, reward: def.reward,
        });
    }
    Ok(out)
}

pub async fn tick_achievement(pool: &PgPool, user_id: &str, achievement_id: &str, increment: i32) -> Result<(), AppError> {
    let defs = catalog::achievement_defs();
    let target = defs.iter().find(|d| d.id == achievement_id).map(|d| d.target).unwrap_or(1);
    sqlx::query(
        "INSERT INTO user_achievements (user_id, achievement_id, progress, unlocked, claimed)
...
pub async fn tick_achievement_tx(tx: &mut Tx<'_>, user_id: &str, achievement_id: &str, increment: i32) -> Result<(), AppError> {
    let defs = catalog::achievement_defs();
    let target = defs.iter().find(|d| d.id == achievement_id).map(|d| d.target).unwrap_or(1);
    sqlx::query(
        "INSERT INTO user_achievements (user_id, achievement_id, progress, unlocked, claimed)
...
pub async fn mark_achievement_claimed_tx(
    ...
    let row = row.ok_or_else(|| AppError::BadRequest("Achievement not unlocked or already claimed".into()))?;
...
#[allow(dead_code)]
pub async fn claim_achievement(pool: &PgPool, user_id: &str, achievement_id: &str) -> Result<Achievement, AppError> {
...
fn end_of_day_utc() -> String {
    ...
    let now = Utc::now();
    let eod = Utc.with_ymd_and_hms(now.year(), now.month(), now.day(), 23, 59, 59)
...
pub async fn get_quests(pool: &PgPool, user_id: &str) -> Result<Vec<DailyQuest>, AppError> {
    ...
    #[derive(sqlx::FromRow)]
    struct Row { quest_id: String, progress: i32, completed: bool, claimed: bool }
    ...
    for def in defs {
        ...
        out.push(DailyQuest {
            id: def.id, name: def.name, description: def.description, emoji: def.emoji,
            progress, target: def.target, completed, claimed,
            reward: def.reward, expires_at: expires.clone(),
        });
    }
    Ok(out)
}
...
pub async fn tick_quest_tx(tx: &mut Tx<'_>, user_id: &str, quest_id: &str) -> Result<(), AppError> {
    let defs = catalog::quest_defs();
    let target = defs.iter().find(|d| d.id == quest_id).map(|d| d.target).unwrap_or(1);
    sqlx::query(
...
pub async fn mark_quest_claimed_tx(
    ...
    let row = row.ok_or_else(|| AppError::BadRequest("Quest not completed or already claimed".into()))?;
...
#[allow(dead_code)]
pub async fn claim_quest(pool: &PgPool, user_id: &str, quest_id: &str) -> Result<DailyQuest, AppError> {
    let mut tx = pool.begin().await?;
...
pub async fn reset_daily_quests(pool: &PgPool) -> Result<u64, AppError> {
    let result = sqlx::query(
        "DELETE FROM user_quests WHERE expires_at < NOW()"
    )
    .execute(pool)
    .await?;
