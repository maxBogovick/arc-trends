...
pub async fn get_command_owner(
    pool: &PgPool,
    command_id: &str,
) -> Result<Option<(String, String)>, AppError> {
    let row = sqlx::query_as::<_, (String, String)>(
        "SELECT pet_id, user_id FROM pet_commands WHERE id = $1",
    )
    .bind(command_id)
    .fetch_optional(pool)
    .await?;
...
pub async fn get_last_accepted_command_id(
    pool: &PgPool,
    pet_id: &str,
) -> Result<Option<String>, AppError> {
    let row = sqlx::query_as::<_, (String,)>(
        "SELECT id FROM pet_commands
         WHERE pet_id = $1 AND status = 'accepted'
...
#[allow(clippy::too_many_arguments)]
pub async fn insert_command_tx(
    tx: &mut Tx<'_>,
    id: &str,
    pet_id: &str,
    user_id: &str,
        ...
        sqlx::query_as::<_, (Value,)>(
            "SELECT result_json FROM pet_commands
             WHERE pet_id = $1 AND status = 'accepted' AND result_json IS NOT NULL
               AND created_at > (
                   SELECT created_at FROM pet_commands
