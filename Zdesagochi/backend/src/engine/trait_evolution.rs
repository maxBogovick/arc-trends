...
fn behavior_signal_for_influence(influence_id: &str) -> HashMap<BehaviorAxis, f64> {
    ...
    let pairs: &[(BehaviorAxis, f64)] = match influence_id {
        ...
        "action:play:active" => &[(Play, 2.2), (Disruption, 0.45)],
        "action:play:puzzle" => &[(Play, 1.1), (Exploration, 1.8), (Order, 1.2)],
        "action:play:social" => &[(Play, 1.5), (Social, 1.6), (Care, 0.3)],
        "action:bond" => &[(Social, 2.0), (Care, 0.5)],
        "action:bond:listen" => &[(Recovery, 2.2), (Social, 1.2)],
        "action:bond:praise" => &[(Social, 1.7), (Care, 0.7)],
        "action:bathe" => &[(Order, 2.0), (Care, 0.5)],
        "action:heal" => &[(Recovery, 2.0), (Care, 1.0)],
        "action:sleep_natural" => &[(Order, 1.2), (Care, 0.6)],
        "action:sleep:ritual" => &[(Order, 1.8), (Recovery, 1.4), (Care, 0.8)],
        "action:sleep:nap" => &[(Recovery, 1.4), (Care, 0.5)],
        "action:sleep_forced" => &[(Disruption, 2.0), (Order, -0.5)],
        "action:wake_early" => &[(Disruption, 2.0), (Order, -1.0)],
        "action:wake:gentle" => &[(Recovery, 0.8), (Social, 0.5), (Care, 0.4)],
